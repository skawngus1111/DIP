function [SmoothImage] = HarmonicMeanFilter(image,windowSize)
...
for i=n+1:height+n % 입력 영상의 높이
    for j=n+1:width+n % 입력 영상이 너비
        ...
        for s=-n:n
            for t=-n:n
                pixelValue = pixelValue + 1 / (kernel(s+n+1, t+n+1)*PaddedImage(i+s, j+t));
            end
        end
        SmoothImage(i-n, j-n) = windowSize^2 / pixelValue;
    end
end
