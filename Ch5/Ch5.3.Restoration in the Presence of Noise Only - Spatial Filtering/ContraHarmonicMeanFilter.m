function [SmoothImage] = ContraHarmonicMeanFilter(image,windowSize, order)
%CONTRAHARMONICMEANFILTER 이 함수의 요약 설명 위치
%   자세한 설명 위치
if size(image, 3) == 3 
...
for i=n+1:height+n % 입력 영상의 높이
    for j=n+1:width+n % 입력 영상이 너비
        ...
        for s=-n:n
            for t=-n:n
                numerator = numerator + (kernel(s+n+1, t+n+1)*PaddedImage(i+s, j+t)) ^ (order + 1);
                denominator = denominator + (kernel(s+n+1, t+n+1)*PaddedImage(i+s, j+t)) ^ order;
            end
        end
        SmoothImage(i-n, j-n) = numerator / denominator;
    end
end
